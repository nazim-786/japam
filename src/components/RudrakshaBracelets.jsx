...
const categoryItems = [
  { title: "Bestsellers", image: "https://images.unsplash.com/photo-1517841905240-472988babdf9?q=80&w=900&auto=format&fit=crop" },
  { title: "Newest Items", image: "https://images.unsplash.com/photo-1524504388940-b1c1722653e1?q=80&w=900&auto=format&fit=crop" },
  { title: "Energy Stones", image: "https://images.unsplash.com/photo-1512436991641-6745cdb1723f?q=80&w=900&auto=format&fit=crop" },
  { title: "Rudraksha Wearables", image: "https://images.unsplash.com/photo-1617038220319-276d3cfab638?q=80&w=900&auto=format&fit=crop" },
  { title: "Karungali Wearables", image: "https://images.unsplash.com/photo-1523170335258-f5ed11844a49?q=80&w=900&auto=format&fit=crop" },
  { title: "Spiritual Jewellery", image: "https://images.unsplash.com/photo-1543294001-f7cd5d7fb516?q=80&w=900&auto=format&fit=crop" },
];

function FilterSection({ title, options, defaultOpen = true, compact = false }) {
          ...
          {options.map((option) => (
            ...
            >
              <span className="flex items-center gap-3">
                <input type="checkbox" className="h-4 w-4 rounded border-[#7c7a7a] accent-[#1f2340]" />
                <span>{option.label}</span>
              </span>
              <span className="text-[#6b6b6b]">({option.count})</span>
            </label>
          ))}
        </div>
      )}
    ...
    <article className="group rounded-[14px] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_10px_28px_rgba(15,23,42,0.08)]">
      ...
      <div className="mt-3 px-1">
        ...
        <div className="mt-2 flex items-center gap-1 text-[#d73c3c]">
          ...
          <span className="ml-1 text-[12px] text-[#3a3d4a]">({product.reviews})</span>
        </div>

        <div className="mt-2 flex items-center gap-2">
          <span className="text-[16px] font-bold text-[#1f2340]">₹{product.price}</span>
          <span className="text-[13px] text-[#7d7d7d] line-through">₹{product.comparePrice}</span>
        </div>
      </div>
    </article>
  );
  ...
  return (
    ...
    >
      {products.map((product) => (
        <div key={product.id} className={viewMode === "list" ? "flex gap-4" : ""}>
...
function DiscoverMore() {
  return (
    <section className="mt-12">
      <h2 className="mb-8 text-[28px] font-bold text-[#1f2340] md:text-[34px]">Discover More</h2>
      <div className="flex gap-5 overflow-x-auto pb-2 md:justify-between md:overflow-visible">
        {categoryItems.map((item) => (
          <div key={item.title} className="min-w-[150px] flex-1 text-center md:min-w-0">
            ...
            <p className="mt-3 text-[15px] font-medium text-[#1f2340] md:text-[16px]">{item.title}</p>
          </div>
        ))}
      </div>
    </section>
...
export default function RudrakshaBraceletsPage() {
    ...
    <div className="min-h-screen bg-[#ffeed1] text-[#1f2340]">
      <div className="mx-auto max-w-[1360px] px-4 pb-12 pt-6 md:px-6 lg:px-8">
        <div className="mb-4 text-[12px] text-[#1f2340] md:text-[13px]">
          ...
          <span className="font-medium">Rudraksha Wearables</span>
        </div>
        ...
        <header className="pt-4 text-center">
          ...
          <p className="mt-2 text-[14px] text-[#2d2f3e] md:text-[18px]">(112 products)</p>
          ...
          <div className="mt-6">
            ...
            <div className="mt-3 flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-[14px] font-medium text-[#1f2340] md:text-[18px]">
              {purposeLinks.map((link) => (
                <button key={link} type="button" className="transition hover:text-[#d83f3f]">
                  {link}
                </button>
              ))}
            </div>
        ...
        <div className="mt-6 flex flex-col gap-3 border-b border-[#d7cbbb] pb-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex items-center justify-end gap-3 text-[#1f2340]">
            <span className="text-[14px] md:text-[15px]">View as</span>
            <div className="flex items-center gap-2">
              <button type="button" onClick={() => setViewMode("list")} className={`rounded-md border p-2 ${viewMode === "list" ? "border-[#1f2340] bg-[#eee7df]" : "border-[#d7cbbb] bg-white"}`}>
                <FaList className="text-[14px]" />
              </button>
              <button type="button" onClick={() => setViewMode("grid")} className={`rounded-md border p-2 ${viewMode === "grid" ? "border-[#1f2340] bg-[#eee7df]" : "border-[#d7cbbb] bg-white"}`}>
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[280px_minmax(0,1fr)]">
          <aside className="hidden lg:block">
            <div className="space-y-1">
              <FilterSection title="Purpose" options={purposeOptions} />
              <div className="py-3">
                <button type="button" className="text-[14px] text-[#1f2340] underline decoration-[#8d8d8d] underline-offset-2">Show more</button>
              </div>

              <div className="border-b border-[#e7dfd2] py-4">
                <div className="mb-4 flex items-center justify-between">
                  <h3 className="text-[15px] font-semibold text-[#1f2340]">Price</h3>
              ...
              <FilterSection title="Bead" options={beadOptions} />
              <FilterSection title="Mukhi" options={mukhiOptions} />
              <div className="py-3">
                <button type="button" className="text-[14px] text-[#1f2340] underline decoration-[#8d8d8d] underline-offset-2">Show more</button>
              </div>
              <FilterSection title="Plating" options={platingOptions} compact />
            </div>
          </aside>
          ...
          <main>
            ...
            <div className="mt-8 flex flex-col items-center justify-center gap-3 pb-4 text-center">
              <p className="text-[15px] text-[#1f2340]">Showing {visibleProducts.length} of {productData.length}</p>
              <div className="h-px w-full max-w-[360px] bg-[#d7cbbb]" />
              <button
                type="button"
                onClick={() => setShowCount((prev) => Math.min(prev + 8, productData.length))}
      ...
      {mobileDrawerOpen && (
        <div className="fixed inset-0 z-50 bg-black/30 lg:hidden" onClick={() => setMobileDrawerOpen(false)}>
          <div className="absolute right-0 top-0 h-full w-[85%] max-w-[330px] overflow-y-auto bg-[#f4efe7] p-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="mb-4 flex items-center justify-between border-b border-[#d7cbbb] pb-3">
              <h3 className="text-[18px] font-bold text-[#1f2340]">Filters</h3>
              <button type="button" onClick={() => setMobileDrawerOpen(false)} className="text-[20px]">×</button>
            </div>
